...
SYSEX_RESPONSE[CAPABILITY_RESPONSE] = function(board) {
  var supportedModes = 0;
    //console.log("7.-Response==>Capability response")
...
var Board = function(port, options, callback) {
  ...
  this.sp.on("data", function(data) {
      //console.log("REcibimos data man");
    var byt, cmd;
    ...
    this.once("queryfirmware", function() {
      ...
      if (settings.skipCapabilities) {
        ready();
          ...
          // console.log("4.-querCapabilities & queryAnalog Mapping")
  ...
  this.once("reportversion", callback);
   ...
   // newFrame.data = REPORT_VERSION;
    newFrame.data=START_SYSEX<<16 | QUERY_FIRMWARE<<8 | END_SYSEX;
      console.log("Report Version,   ", newFrame.data/*.toString(16)*/);
    //xbeeAPI.buildFrame(construyendoFrames([REPORT_VERSION]))                   .toString(16)
    console.log(new Buffer(xbeeAPI.buildFrame(newFrame)));
  this.sp.write(Buffer(xbeeAPI.buildFrame(newFrame)));
};

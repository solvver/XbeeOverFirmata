...
/*exports.addParser = function (Xbee){
    ...
    Xbee._*/
frame_parser["ARTURO"] = function (frame, buffer) {
        console.log("Using our parser", buffer.toString("HEX"))
       // frame.temperatura=buffer.readUInt8(9);
        frame.RSSI=buffer.readUInt8(7);
        frame.temperatura=buffer.readUInt8(6);
        frame.numeroMuestra=buffer.readUInt8(1);
       // if (buffer[0]==0x11 && buffer[1]==0x11) {frame.RSSI=buffer.readUInt8(10);
       //   console.log("RSSI", frame.temperatura);
        //}

        console.log("Número de muestra:", frame.numeroMuestra);
        console.log("Temperatura:", frame.temperatura);
        console.log("RSSI", frame.RSSI)



};

frame_parser["getRssi"] = function (frame, buffer) {
    console.log("getRssi", buffer.toString("HEX"))
    console.log(buffer.length);
    frame.numeroMuestra=buffer.readUInt8(1);
   /* setTimeout(serialport.write("+++"), 1000);
    setTimeout(serialport.write("ATDB"), 1000);
    serialport.on("data", function (data) {
    frame.rssi=data;
    });
    setTimeout(serialport.write("ATCN"), 1000)*/
    setTimeout(console.log("Número de muestra:", frame.numeroMuestra)
    , 1000);
    //console.log("RSSI", frame.rssi)
};
